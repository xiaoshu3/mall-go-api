...
func (p Paginator) getPerPage(perPage int) int {
	// 优先使用请求 per_page 参数
	queryPerpage := p.ctx.Query(config.Get("paging.url_query_per_page")) //per_page
...
func (p Paginator) getCurrentPage() int {
	// 优先取用户请求的 page
	page := cast.ToInt(p.ctx.Query(config.Get("paging.url_query_page"))) //page
	if page <= 0 {
		// 默认为 1
		page = 1
	}
...
func (p Paginator) getPageLink(page int) string {
	return fmt.Sprintf("%v%v&%s=%s&%s=%s&%s=%v",
		p.BaseURL,
		page,
		config.Get("paging.url_query_sort"), //sort
		p.Sort,
		config.Get("paging.url_query_order"), //order
		p.Order,
		config.Get("paging.url_query_per_page"), //per_page
		p.PerPage,
	)
}
